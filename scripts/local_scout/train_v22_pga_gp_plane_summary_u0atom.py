...
class MotorAttention(nn.Module):
    def __init__(self, d_model: int, n_heads: int, n_rbf: int = 20, plane_summary_hidden: int = 32):
        super().__init__()
        ...
        self.plane_summary_bias = nn.Sequential(
            nn.LayerNorm(10),
            nn.Linear(10, int(plane_summary_hidden)),
            nn.SiLU(),
            nn.Linear(int(plane_summary_hidden), n_heads),
        )
        nn.init.zeros_(self.plane_summary_bias[-1].weight)
        nn.init.zeros_(self.plane_summary_bias[-1].bias)
    ...
    def forward(
        ...
        pair_plane_feat: torch.Tensor | None,
        mask: torch.Tensor,
        ...
        if pair_plane_feat is not None:
            pf = torch.log1p(pair_plane_feat.float().clamp_min(0.0)).to(dtype=x_f32.dtype)
            scores = scores + self.plane_summary_bias(pf).permute(0, 3, 1, 2)
...
class V20BlockMotor(nn.Module):
    def __init__(self, d_model: int, n_heads: int, n_rbf: int = 20, dropout: float = 0.0, plane_summary_hidden: int = 32):
        super().__init__()
        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)
        self.attn = MotorAttention(d_model=d_model, n_heads=n_heads, n_rbf=n_rbf, plane_summary_hidden=plane_summary_hidden)
    ...
    def forward(
        ...
        h = self.norm1(x)
        attn_out, sig = self.attn(h, rbf_feat, coulomb_feat=coulomb_feat, pos_dense=pos_dense, pair_plane_feat=pair_plane_feat, mask=mask, motor_strength=motor_strength)
...
class V20AGAAMotor(nn.Module):
    def __init__(
        ...
        *,
        ...
        plane_summary_hidden: int = 32,
    ):
        super().__init__()
        self.d_model = int(d_model)
        self.n_heads = int(n_heads)
        self.use_coulomb = bool(use_coulomb)
        self.pair_plane_summary_cpu = None
        ...
        self.layers = nn.ModuleList([
            V20BlockMotor(d_model=d_model, n_heads=n_heads, n_rbf=n_rbf, dropout=dropout, plane_summary_hidden=plane_summary_hidden)
    ...
    def forward(self, data, motor_strength: float = 0.0) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        pair_plane_feat = None
        if self.pair_plane_summary_cpu is not None and hasattr(data, "mol_idx"):
            mol_idx = data.mol_idx.view(-1).detach().cpu().long()
            Bp, Np = z_dense.shape
            pair_plane_feat = self.pair_plane_summary_cpu[mol_idx, :Np, :Np, :].to(
                device=pos_dense.device,
                dtype=pos_dense.dtype,
                non_blocking=True,
            )
        ...
        for layer in self.layers:
            h, sig = layer(h, rbf_feat, coulomb_feat, pos_dense, pair_plane_feat, mask, motor_strength=float(motor_strength))
...
def main():
    ...
    ap.add_argument("--pair-plane-cache", default=None, help="Optional precomputed PGA triplet-plane pair summary cache.")
    ap.add_argument("--plane-summary-hidden", type=int, default=32)
    ...
    model = V20AGAAMotor(
        ...
        plane_summary_hidden=int(args.plane_summary_hidden),
    ).to(device)

    if args.pair_plane_cache:
        print({"loading_pair_plane_cache": str(args.pair_plane_cache)}, flush=True)
        cache_obj = torch.load(args.pair_plane_cache, map_location="cpu")
        model.pair_plane_summary_cpu = cache_obj["pair_plane_summary"].contiguous()
        print({
            "pair_plane_summary_shape": list(model.pair_plane_summary_cpu.shape),
            "pair_plane_summary_dtype": str(model.pair_plane_summary_cpu.dtype),
        }, flush=True)
    ...
    metadata = {
        ...
        "method": "standalone V22/PGA-GP plus triplet-plane-summary scout",
        "target": TARGET_NAME,
