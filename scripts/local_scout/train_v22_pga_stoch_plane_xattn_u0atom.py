...
def _quat_conj(q: torch.Tensor) -> torch.Tensor:
    """Quaternion conjugate. Basis convention: [1, e23, e31, e12]."""
    return torch.cat([q[..., :1], -q[..., 1:]], dim=-1)


def _quat_mul(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    """
    Hamilton product with cyclic bivector convention:
      i = e23, j = e31, k = e12
      i*j=k, j*k=i, k*i=j.
    Inputs end in dimension 4: [scalar, x_e23, y_e31, z_e12].
    """
...
def _dq_mul(ar: torch.Tensor, ad: torch.Tensor, br: torch.Tensor, bd: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """Dual quaternion / PGA motor product: (ar + eps ad)(br + eps bd)."""
    rr = _quat_mul(ar, br)
...
def _dq_reverse(r: torch.Tensor, d: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Motor reverse approximation used for attention compatibility.
    For this first POC we reverse/conjugate primal and dual quaternions.
    """
...
def _pga_points_cyclic(pos: torch.Tensor) -> torch.Tensor:
    """
    PGA point/trivector coordinates using requested cyclic convention:

      P(x,y,z) = e123 + x e023 + y e031 + z e012
...
class MotorAttention(nn.Module):
    ...
    def forward(
        ...
        # V22-PGA-GP-M0:
        # Treat each 8D q/k as a PGA motor / dual quaternion:
        #   M = r + eps d
        # with basis convention:
        #   r,d = [scalar, e23, e31, e12]
        #
        # Atom points use the requested cyclic trivector convention:
        #   P = e123 + x e023 + y e031 + z e012
        #
        # Pairwise relative translator:
        #   T_ij = 1 + eps * 0.5 * (0, p_i - p_j)
        #
        # Attention motor compatibility:
        #   R_ij = Q_i T_ij reverse(K_j)
        #   score = scalar(R_ij.primal) + eta * scalar(R_ij.dual)
        q_r = q_raw[..., :4]
        q_d = q_raw[..., 4:]
        k_r = k_raw[..., :4]
        k_d = k_raw[..., 4:]
        ...
        qi_r = q_r.unsqueeze(3)  # [B,H,N,1,4]
        qi_d = q_d.unsqueeze(3)
        kj_r = k_r.unsqueeze(2)  # [B,H,1,N,4]
        kj_d = k_d.unsqueeze(2)

        if pos_dense is None:
            # Fallback: no relative translator, pure motor product.
            t_r = torch.zeros_like(qi_r + kj_r)
            t_r[..., 0] = 1.0
            t_d = torch.zeros_like(t_r)
        else:
            # pos_dense order is Euclidean xyz, interpreted as cyclic PGA point:
            # P = e123 + x e023 + y e031 + z e012.
            _ = _pga_points_cyclic(pos_dense)  # explicit convention anchor / debug no-op
            rel = pos_dense[:, :, None, :].float() - pos_dense[:, None, :, :].float()  # [B,N,N,3]
            rel = rel.unsqueeze(1)  # [B,1,N,N,3]
        ...
        primal_scalar = rij_r[..., 0]
        dual_scalar = rij_d[..., 0].clamp(min=-5.0, max=5.0)

        motor_score = 8.0 * (primal_scalar + 0.010 * dual_scalar)
...
class V20BlockMotor(nn.Module):
    ...
    def forward(
        ...
        h = self.norm1(x)
        attn_out, sig = self.attn(h, rbf_feat, coulomb_feat=coulomb_feat, pos_dense=pos_dense, mask=mask, motor_strength=motor_strength)
...
class V20AGAAMotor(nn.Module):
    def __init__(
        ...
        *,
        ...
        plane_context_hidden: int = 64,
        plane_context_scale: float = 1.0,
    ):
        super().__init__()
        self.d_model = int(d_model)
        self.n_heads = int(n_heads)
        self.use_coulomb = bool(use_coulomb)
        self.stoch_plane_cache_cpu = None
        self.plane_context_scale = float(plane_context_scale)
        self.plane_token_mlp = nn.Sequential(
            nn.LayerNorm(9),
            nn.Linear(9, int(plane_context_hidden)),
            nn.SiLU(),
            nn.Linear(int(plane_context_hidden), d_model),
        )
        self.plane_context_proj = nn.Linear(d_model, d_model)
        nn.init.zeros_(self.plane_context_proj.weight)
        nn.init.zeros_(self.plane_context_proj.bias)
        self.atom_plane_q = nn.Linear(d_model, d_model, bias=False)
        self.atom_plane_k = nn.Linear(d_model, d_model, bias=False)
        self.atom_plane_v = nn.Linear(d_model, d_model, bias=False)
        self.atom_plane_out = nn.Linear(d_model, d_model, bias=False)
        nn.init.zeros_(self.atom_plane_out.weight)
    ...
    def forward(self, data, motor_strength: float = 0.0) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for layer in self.layers:
            h, sig = layer(h, rbf_feat, coulomb_feat, pos_dense, mask, motor_strength=float(motor_strength))
            motor_sig_total = motor_sig_total + sig

        h = self.norm_final(h)
        if self.stoch_plane_cache_cpu is not None and hasattr(data, "mol_idx"):
            mol_idx = data.mol_idx.view(-1).detach().cpu().long()
            cache = self.stoch_plane_cache_cpu

            planes = cache["planes"][mol_idx].to(device=h.device, dtype=h.dtype, non_blocking=True)
            areas = cache["areas"][mol_idx].to(device=h.device, dtype=h.dtype, non_blocking=True)
            zstats = cache["zstats"][mol_idx].to(device=h.device, dtype=h.dtype, non_blocking=True)
            pmask = cache["mask"][mol_idx].to(device=h.device, dtype=torch.bool, non_blocking=True)
            trip_idx = cache["triplet_indices"][mol_idx].to(device=h.device, dtype=torch.long, non_blocking=True)

            zsum = zstats[..., 0:1] / 30.0
            zmax = zstats[..., 1:2] / 10.0
            heavy = zstats[..., 2:3] / 3.0
            area_from_zstats = torch.log1p(zstats[..., 3:4].clamp_min(0.0))
            area_feat = torch.log1p(areas.unsqueeze(-1).clamp_min(0.0))

            plane_feat = torch.cat(
                [planes, area_feat, zsum, zmax, heavy, area_from_zstats],
                dim=-1,
            )

            plane_emb = self.plane_token_mlp(plane_feat)

            q = self.atom_plane_q(h)
            k = self.atom_plane_k(plane_emb)
            v = self.atom_plane_v(plane_emb)

            scores = torch.einsum("bnd,bkd->bnk", q, k) / math.sqrt(float(h.size(-1)))
            scores = scores.masked_fill(~pmask[:, None, :], -1e9)

            # Incidence bias: atom i is allowed to prefer planes that contain atom i.
            Bp, Np = h.shape[:2]
            atom_ids = torch.arange(Np, device=h.device).view(1, Np, 1, 1)
            incidence = (trip_idx[:, None, :, :] == atom_ids).any(dim=-1)
            scores = scores + 0.25 * incidence.float()

            attn = torch.softmax(scores, dim=-1)
            plane_ctx_atom = torch.einsum("bnk,bkd->bnd", attn, v)

            h = h + self.plane_context_scale * self.atom_plane_out(plane_ctx_atom) * mask_f
...
def indexed_subset(dataset, indices):
    out = []
    for i in indices:
        ii = int(i)
        d = dataset[ii]
        d.mol_idx = torch.tensor([ii], dtype=torch.long)
        out.append(d)
    return out
...
def main():
    ...
    ap.add_argument("--stoch-plane-cache", default=None, help="Optional stochastic sampled triplet-plane cache.")
    ap.add_argument("--plane-context-hidden", type=int, default=64)
    ap.add_argument("--plane-context-scale", type=float, default=1.0)
    ...
    train_loader = DataLoader(
        indexed_subset(dataset, train_idx),
        batch_size=args.batch_size,
        shuffle=True,
        num_workers=args.num_workers,
    )
    val_loader = DataLoader(
        indexed_subset(dataset, val_idx),
    ...
    model = V20AGAAMotor(
        ...
        plane_context_hidden=int(args.plane_context_hidden),
        plane_context_scale=float(args.plane_context_scale),
    ).to(device)

    if args.stoch_plane_cache:
        print({"loading_stoch_plane_cache": str(args.stoch_plane_cache)}, flush=True)
        cache_obj = torch.load(args.stoch_plane_cache, map_location="cpu")
        model.stoch_plane_cache_cpu = {
            "planes": cache_obj["planes"].contiguous(),
            "areas": cache_obj["areas"].contiguous(),
            "zstats": cache_obj["zstats"].contiguous(),
            "mask": cache_obj["mask"].contiguous(),
            "triplet_indices": cache_obj["triplet_indices"].contiguous(),
        }
        print({
            "stoch_planes_shape": list(model.stoch_plane_cache_cpu["planes"].shape),
            "stoch_planes_dtype": str(model.stoch_plane_cache_cpu["planes"].dtype),
        }, flush=True)
    ...
    metadata = {
        ...
        "method": "standalone V22/PGA-GP plus stochastic atom-to-plane attention scout",
        "target": TARGET_NAME,
