...
class MotorAttention(nn.Module):
    ...
    def forward(
        ...
        # Enforce the simplest motor constraints:
        #   ||r|| = 1
        #   <r, d> = 0
        # This is not yet full PGA sandwich propagation, but it is a controlled
        # step from unconstrained V20 q_screw/k_screw vectors toward normalized
        # PGA motor objects.
        q_r = q_raw[..., :4]
        q_d = q_raw[..., 4:]
        k_r = k_raw[..., :4]
        ...
        q_d = q_d / q_d.norm(dim=-1, keepdim=True).clamp_min(1e-8)
        k_d = k_d / k_d.norm(dim=-1, keepdim=True).clamp_min(1e-8)

        rot_score = (q_r.unsqueeze(3) * k_r.unsqueeze(2)).sum(dim=-1)
        dual_score = (q_d.unsqueeze(3) * k_d.unsqueeze(2)).sum(dim=-1)

        motor_score = rot_score + 0.10 * dual_score
...
def main():
    ...
    metadata = {
        ...
        "method": "standalone V21/PGA normalized-motor scout",
        "target": TARGET_NAME,
