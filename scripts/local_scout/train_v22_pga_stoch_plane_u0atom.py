...
class V20AGAAMotor(nn.Module):
    ...
    def forward(self, data, motor_strength: float = 0.0) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.stoch_plane_cache_cpu is not None and hasattr(data, "mol_idx"):
            ...
            planes = cache["planes"][mol_idx].to(device=graph_h.device, dtype=graph_h.dtype, non_blocking=True)
            areas = cache["areas"][mol_idx].to(device=graph_h.device, dtype=graph_h.dtype, non_blocking=True)
            zstats = cache["zstats"][mol_idx].to(device=graph_h.device, dtype=graph_h.dtype, non_blocking=True)
            pmask = cache["mask"][mol_idx].to(device=graph_h.device, dtype=torch.bool, non_blocking=True)

            # plane: [d, nx, ny, nz], already approximately unit-normalized in cache
            # area/z stats are compressed to stable scalar features.
            ...
            pmask_f = pmask.float().unsqueeze(-1)
            plane_ctx = (plane_emb * pmask_f).sum(dim=1) / pmask_f.sum(dim=1).clamp_min(1.0)

            graph_h = graph_h + self.plane_context_scale * self.plane_context_proj(plane_ctx)
...
def main():
    ...
    metadata = {
        ...
        "method": "standalone V22/PGA-GP plus stochastic triplet-plane context scout",
        "target": TARGET_NAME,
