...
class ChemicallyConditionedMetric(nn.Module):
    ...
    def forward(self, x_head, rij_r, rij_d, pair_mask, z_dense):
        ...
        # 1. Chemical Tikhonov Prior (Valence-Specific Regularization)
        log_lmbda_local = self.lambda_proj(chem_pair).view(B, 1, N, N, 1, 1)
        lmbda = torch.exp(self.log_lmbda_base + log_lmbda_local)

        # 2. Valence-Gated Tangent Projections
        U_raw = self.U_proj(x_head).view(B, N, H, 8, self.rank).permute(0, 2, 1, 3, 4)
        gate = torch.sigmoid(self.u_gate(chem_pair)).view(B, 1, N, N, 8, self.rank)
        
        U_ij = U_raw.unsqueeze(3) * gate
        U = torch.tanh(U_ij) * self.metric_scale
        U_T = U.transpose(4, 5)

        # Edge-specific Motor Projection
...
class MotorAttention(nn.Module):
    def __init__(self, d_model: int, n_heads: int, n_rbf: int = 20):
        ...
        self.info_metric = ChemicallyConditionedMetric(
    ...
    def forward(
        ...
        motor_score = self.info_metric(x_head, rij_r, rij_d, pair_mask, z_dense)
...
class V20BlockMotor(nn.Module):
    ...
    def forward(
        ...
        h = self.norm1(x)
        attn_out, sig = self.attn(h, rbf_feat, coulomb_feat=coulomb_feat, pos_dense=pos_dense, mask=mask, z_dense=z_dense, motor_strength=motor_strength)
...
def main():
    ...
    metadata = {
        ...
        "method": "V25 Chemically Conditioned Information Metric",
        "target": TARGET_NAME,
