...
class GradedDualQuaternionMetric(nn.Module):
    def __init__(self, hidden_dim=192, n_heads=16, rank=8, delta=0.5, metric_scale=2.0):
        super().__init__()
        ...
        self.log_lmbda = nn.Parameter(torch.tensor([math.log(0.005)]))
        
        # Balance the PGA Subalgebras: Primal (Rotations) vs Dual (Translations)
    ...
    def forward(self, x_head, rij_r, rij_d, pair_mask):
        ...
        D_ij = torch.sum(projected_motor**2, dim=-1) + lmbda * torch.sum(M_ij**2, dim=-1)
...
class MotorAttention(nn.Module):
    def __init__(self, d_model: int, n_heads: int, n_rbf: int = 20):
        ...
        self.info_metric = GradedDualQuaternionMetric(
...
def main():
    ...
    metadata = {
        ...
        "method": "standalone V24 Graded Dual Quaternion Metric",
        "target": TARGET_NAME,
