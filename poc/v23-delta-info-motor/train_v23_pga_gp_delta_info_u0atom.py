...
# =============================================================================
# Robust Delta Information Separation Utils
# =============================================================================
...
def _pga_points_cyclic(pos: torch.Tensor) -> torch.Tensor:
    ...
    Returned order: [e123, e023, e031, e012]
    """
    one = torch.ones_like(pos[..., :1])
    return torch.cat([one, pos[..., 0:1], pos[..., 1:2], pos[..., 2:3]], dim=-1)

# =============================================================================
# Dual Quaternion Information Metric (8D PGA)
# =============================================================================
class DualQuaternionInformationMetric(nn.Module):
    def __init__(self, hidden_dim=192, n_heads=16, rank=8, lmbda=0.005, delta=1.0, metric_scale=2.0):
        super().__init__()
        ...
        # Tangent projection: Maps the node states to the 8D dual quaternion metric space
        self.U_proj = nn.Linear(self.d_head, 8 * rank)

    def forward(self, x_head, rij_r, rij_d, pair_mask):
        B, N, H, D = x_head.shape
        
        # Recombine primal and dual components into 8D motor M_ij
        M_ij = torch.cat([rij_r, rij_d], dim=-1)

        # Compute Tangent Space U
        U_raw = self.U_proj(x_head).view(B, N, H, 8, self.rank).permute(0, 2, 1, 3, 4)
        U = torch.tanh(U_raw) * self.metric_scale
        U_T = U.transpose(3, 4)

        # Project Relative Motor
        projected_motor = torch.einsum('bhird, bhijd -> bhijr', U_T, M_ij)
        
        # Delta Information Separation Distance
        D_ij = torch.sum(projected_motor**2, dim=-1) + self.lmbda * torch.sum(M_ij**2, dim=-1)

        # Float32 Stable Information Volume
        I_R = torch.eye(self.rank, device=x_head.device).view(1, 1, 1, self.rank, self.rank)
        U_T_U = torch.einsum('bhird, bhidc -> bhirc', U_T, U)
        
        logdet = torch.linalg.slogdet(self.lmbda * I_R + U_T_U)[1]
        log_vol_i = 0.5 * logdet.unsqueeze(3)
        
        temp = torch.exp(self.log_temp) + 1e-4
        vol_weight = torch.exp(self.log_vol_weight)
        
        attention_logits = (-0.5 * D_ij / temp) - (vol_weight * log_vol_i)
...
class MotorAttention(nn.Module):
    def __init__(self, d_model: int, n_heads: int, n_rbf: int = 20):
        ...
        # The Drop-in Information Metric
        self.info_metric = DualQuaternionInformationMetric(
            hidden_dim=d_model, n_heads=n_heads, rank=8, delta=1.0
        )
    ...
    def forward(
        ...
        # -------------------------------------------------------------
        # V23 UPDATE: Continuous SE(3) Volume Evaluation
        # -------------------------------------------------------------
        x_head = x_f32.view(B, N, self.n_heads, self.d_head)
        pair_mask = mask[:, None, :, None] & mask[:, None, None, :]
        
        motor_score = self.info_metric(x_head, rij_r, rij_d, pair_mask)
        # -------------------------------------------------------------
        ...
        # Apply robust Delta Deformation
...
def main():
    ...
    metadata = {
        ...
        "method": "standalone V23/PGA-GP true motor-product with Delta Information",
        "target": TARGET_NAME,
